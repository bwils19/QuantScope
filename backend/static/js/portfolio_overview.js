...
function renderWatchlistItems(data = null) {
    ...
    // Set up event listeners for buttons AFTER they're added to the DOM
    document.querySelectorAll('.view-chart-btn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            const symbol = this.getAttribute('data-symbol');
            renderChartForSecurity(symbol);
        });
    });
...
async function loadChartJsLibraries() {
    ...
    if (typeof window.luxon === 'undefined') {
        await loadScript('https://cdn.jsdelivr.net/npm/luxon@3.0.1/build/global/luxon.min.js');
        console.log('Luxon loaded');
    }
    
    if (!Chart.adapters._adapters.date) {
        await loadScript('https://cdn.jsdelivr.net/npm/chartjs-adapter-luxon@1.3.0/dist/chartjs-adapter-luxon.min.js');
        console.log('Chart.js date adapter loaded');
    }
    
    // Finally load the financial charts plugin
    if (typeof Chart.controllers.candlestick === 'undefined') {
        ...
        console.log('Chart.js Financial loaded');
    }
...
async function renderChartForSecurity(symbol) {
    ...
    if (!modalContainer) {
        // Create a simple modal with fixed dimensions
        const modalHTML = `
            <div id="watchlistChartModal" class="modal" style="display: none;">
                <div class="modal-content chart-modal-content" style="width: 90%; max-width: 1200px; margin: 2% auto;">
                    ...
                    <div style="width: 100%; height: 500px; position: relative;">
                        <div id="watchlistChart" style="width: 100%; height: 100%;"></div>
                    </div>
                </div>
            </div>
        `;
        ...
        modalContainer = document.getElementById('watchlistChartModal');
    ...
    // Two-step process: First display modal, then initialize chart
    await new Promise(resolve => setTimeout(resolve, 300)); // Crucial delay
    ...
    try {
        ...
        if (!data || !data.dates || data.dates.length === 0) {
            document.getElementById('watchlistChart').innerHTML = 
        ...
        // Setup chart toggle buttons (after modal is visible)
        ...
        // Render initial chart (line)
        renderWatchlistChart('line');
        
    } catch (error) {
        console.error('Error:', error);
        document.getElementById('watchlistChart').innerHTML = 
            `<div style="text-align: center; padding: 20px; color: red;">Error: ${error.message}</div>`;
    }
}
...
function safelyDestroyChart() {
    if (watchlistChart !== null) {
        try {
            if (typeof watchlistChart.destroy === 'function') {
                watchlistChart.destroy();
...
function renderWatchlistChart(type) {
    console.log(`Rendering ${type} chart with ApexCharts`);
    
    const chartElement = document.getElementById('watchlistChart');
    
    // Log dimensions to verify container is properly sized
    ...
    try {
        ...
        console.error("Error rendering chart:", error);
        chartElement.innerHTML = `<div style="text-align: center; color: red; padding: 20px;">Error: ${error.message}</div>`;
    }
}
...
function loadFinancialChartPlugin() {
    if (typeof Chart.controllers.candlestick === 'undefined' || typeof Chart.controllers.ohlc === 'undefined') {
        // Check if we need to load the plugin
        if (!document.getElementById('chartjs-financial')) {
            console.log('Loading Chart.js Financial plugin...');
            
            // Create script element to load the plugin
            const script = document.createElement('script');
            script.id = 'chartjs-financial';
            script.src = 'https://cdn.jsdelivr.net/npm/chartjs-chart-financial/dist/chartjs-chart-financial.min.js';
            script.async = true;
            ...
            return new Promise((resolve) => {
                script.onload = () => {
                    console.log('Chart.js Financial plugin loaded successfully');
                    resolve();
                };
                script.onerror = () => {
                    console.error('Failed to load Chart.js Financial plugin');
                    // Fall back to line chart in case of error
                    renderWatchlistChart('line');
                };
            });
        }
    }
    return Promise.resolve(); // Plugin already loaded
}
...
function renderChart(type) {
    // Ensure we have data
    if (!window.currentChartData || !window.currentSymbol) {
        console.error('No chart data available');
        return;
    }
    ...
    // Format OHLC data if available (for candlestick)
    const ohlcData = [];
    ...
    if (data.open_prices && data.high_prices && data.low_prices) {
        ...
        // Create simulated OHLC from close prices
        for (let i = 0; i < data.dates.length; i++) {
            const closePrice = data.prices[i];
            const prevClose = i > 0 ? data.prices[i-1] : closePrice;
            
            // Simple simulation - not ideal but works for visualization
            const variation = closePrice * 0.01; // 1% variation
            const open = prevClose;
            const high = Math.max(open, closePrice) + variation;
            const low = Math.min(open, closePrice) - variation;
    ...
    // Chart options - base configuration
    const options = {
        chart: {
            type: 'line', // Default, will be overridden
            height: 400,
            animations: {
                enabled: true,
                easing: 'easeinout',
        ...
        colors: ['#2E8BC0', '#6C7D93', '#4CAF50', '#F44336']
    };
                ...
                custom: function({ seriesIndex, dataPointIndex, w }) {
                    ...
                    return `
                        <div class="apexcharts-tooltip-candlestick">
                            ...
                            <div><b>Open:</b> $${o.toFixed(2)}</div>
                            <div><b>High:</b> $${h.toFixed(2)}</div>
                            <div><b>Low:</b> $${l.toFixed(2)}</div>
                            <div><b>Close:</b> $${c.toFixed(2)}</div>
                        </div>
                    `;
                }
            };
    ...
    if (typeof ApexCharts !== 'undefined') {
        ...
        chartElement.innerHTML = 'ApexCharts library not loaded';
...
async function renderChartForSecurity(symbol) {
    ...
    // Remove any existing modal first to ensure clean state
    const existingModal = document.getElementById('watchlistChartModal');
    if (existingModal) {
        safelyDestroyChart();
        existingModal.remove();
    }
    
    // Create a completely new modal with fixed size chart container
    const modalHTML = `
        <div id="watchlistChartModal" class="modal" style="display: block; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4);">
            <div class="modal-content" style="background-color: #fefefe; margin: 5% auto; padding: 20px; border: 1px solid #888; width: 90%; max-width: 1200px; position: relative;">
                <span class="close" style="color: #aaa; float: right; font-size: 28px; font-weight: bold; cursor: pointer; position: absolute; top: 10px; right: 20px;">&times;</span>
                ...
                <div id="chart-container" style="width: 100%; height: 500px; background-color: #fff; border: 1px solid #eee; border-radius: 4px;"></div>
            </div>
        </div>
    `;
    ...
    document.querySelector('#watchlistChartModal .close').addEventListener('click', () => {
        safelyDestroyChart();
        document.getElementById('watchlistChartModal').remove();
    });
    
    // Setup window click handler for closing the modal
    window.addEventListener('click', function(event) {
        const modal = document.getElementById('watchlistChartModal');
    ...
    document.querySelectorAll('.chart-type-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            this.style.backgroundColor = '#6C7D93';
    ...
    if (!data || !data.dates || data.dates.length === 0) {
        document.getElementById('chart-container').innerHTML = 
            '<div style="display: flex; justify-content: center; align-items: center; height: 100%; color: #e74c3c;">No data available for this security</div>';
        return;
    }
    ...
    // Create chart-container div for ApexCharts
    const chartContainer = document.getElementById('chart-container');
    chartContainer.innerHTML = '';
    
    // Create a specific element for the chart with explicit dimensions
    const chartElement = document.createElement('div');
    chartElement.id = 'watchlist-apex-chart';
    chartElement.style.width = '100%';
    ...
    function renderChart(type) {
        ...
        // Get mounted dimensions
        const container = document.getElementById('watchlist-apex-chart');
        console.log('Chart container dimensions:', container.offsetWidth, container.offsetHeight);
        
        // If still zero dimensions, force with inline styles
        if (container.offsetWidth === 0 || container.offsetHeight === 0) {
            console.warn('Container still has zero dimensions - setting explicit styles');
        ...
        try {
            ...
            // Create and render chart
            watchlistChart = new ApexCharts(container, options);
            watchlistChart.render();
            console.log('Chart rendered successfully');
            
        } catch (error) {
            console.error('Error rendering chart:', error);
            document.getElementById('watchlist-apex-chart').innerHTML = 
                `<div style="display: flex; justify-content: center; align-items: center; height: 100%; color: #e74c3c;">Error: ${error.message}</div>`;
        }
    }
}
    ...
    try {
        ...
        if (type === 'line') {
            ...
            if (currentChartData.open_prices && currentChartData.high_prices && currentChartData.low_prices) {
                ...
                // Generate synthetic OHLC data if only close prices are available
                for (let i = 0; i < currentChartData.dates.length; i++) {
                    const closePrice = currentChartData.prices[i];
                    const prevClose = i > 0 ? currentChartData.prices[i-1] : closePrice;
                    
                    // Simple simulation with small volatility
                    const volatility = closePrice * 0.02;
                    const open = prevClose;
                    const high = Math.max(open, closePrice) + (Math.random() * volatility);
                    const low = Math.min(open, closePrice) - (Math.random() * volatility);
